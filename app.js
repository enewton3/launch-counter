...
async function get5launches() {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      let launchType = launchData[i].mission
        ? launchData[i].mission.type
        : "Check back later for details";
      let launchImage = launchData[i].image;
      let launchURL = launchData[i].url;
      let launchTime = launchData[i].net;
      let launchDetails = launchData[i].mission
        ? launchData[i].mission.description
        : "Check back later for details";
      let thisLaunch = new launch(
        launchID,
        launchName,
        launchType,
        launchImage,
...
function displayLaunches(arr) {
  arr.forEach((item, id) => {
    if (checkForDupes(item)) {
      ...
      intervalArr[id] = new intervalObj(
        itemID,
        setInterval(() => {
          clock.textContent = countdown(clock.id);
        }, 1000)
      );
      let removeButton = counterDiv.querySelector(".remove");
      removeButton.addEventListener("click", (e) =>
        removeDiv(counterDiv, itemID, e)
      );
...
function removeDiv(div, id, e) {
  e.stopPropagation();
  div.remove();
  fiveLaunches = fiveLaunches.filter((item) => {
    return item.id !== id;
  });
  searchedLaunches = searchedLaunches.filter((item) => {
    return item.id !== id;
  });
  fiveEvents = fiveEvents.filter((item) => {
    return item.id !== id;
  });
}
...
function saveLocal() {
  fiveLaunches.forEach((item) => {
    saveThisArray.push(item);
  });
  searchedLaunches.forEach((item) => {
    saveThisArray.push(item);
  });
  fiveEvents.forEach((item) => {
    saveThisArray.push(item);
  });
  fiveLaunches = [];
  searchedLaunches = [];
